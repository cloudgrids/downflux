...
const imports = parserServices.map((s) => `import { ${s.name}Parser } from './${s.name}Parser';`).join('\n');
...
const tImports = transformers.map((s) => `import { ${s.name}Transformer } from './${s.name}Transformer';`).join('\n');
