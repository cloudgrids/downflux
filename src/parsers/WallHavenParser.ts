...
import { DefaultExtractorResult, WallHavenOutput, WallHavenUserFavoriteCollectionsOutput } from '@app/contracts';
import { GenericException } from '@app/exceptions';
import { ProviderType, WallHavenThumbnailQuality } from '@app/shared';
import { DefaultParser } from './DefaultParser';

export class WallHavenParser extends DefaultParser {
	public override transform(html: string, sourceUrl: string): Partial<DefaultExtractorResult<Partial<WallHavenOutput>>> {
		...
		try {
			...
			throw new GenericException('Unable to parse some fields:', ProviderType.WallHaven, 'WallHavenParser', { cause: error });
		}
	}
