import { DownloadOptions, HLSStreamRequest, PipelineItem } from '@app/contracts';
import { DownloadException, NotFoundException } from '@app/exceptions';
...
export class StreamHttpClient extends BaseHttpClient {
	...
	private async handleServiceAware404(url: string, opts: DownloadOptions): Promise<HLSStreamRequest | null> {
		const strategy = this.strategyRegistry.getStrategy(opts.provider);
